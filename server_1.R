...
server <- function(input, output, session) {
  ...
  output$denplot_sidebar <- renderUI({
    ...
    tagList(
      ...
      conditionalPanel(
        ...
        numericInput("binwidth","Width of bin", value = 5)
      ),
  ...
  plot3 <- reactive({
    ...
    Dens.plot(data = df, x_var = input$den_x_var, group.by = input$den_group, col_by_line = input$den_col_by_line,
              ...
              add_legend = input$den_lgd, x_lab = input$den_x_lab, y_lab = input$den_y_lab, binwidth = input$binwidth,
  ...
  plot6 <- reactive({
    ...
    p <- ComplexHeatmap::Heatmap(df, col = col_fun, cluster_rows = input$cluster_rows,
